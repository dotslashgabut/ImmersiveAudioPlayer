        ...
        for (let i = startI; i <= endI; i++) {
            ...
            if (line) {
                ...
                if (isCurrent && transEffect !== 'none') {
                    const elapsed = time - line.time, transDur = 0.5;
                    const p = elapsed < transDur ? elapsed / transDur : 1;
                    if (p < 1) {
                        alpha = Math.min(1, p * 2);
                        if (transEffect === 'slide') offsetY = (1 - p) * 30;
                        else if (transEffect === 'zoom') { const s = 0.5 + (p * 0.5); scaleValX = s; scaleValY = s; }
                        else if (transEffect === 'blur') blurVal = (1 - p) * 10;
                        else if (transEffect === 'float') offsetY = (1 - p) * 50;
                    }
                }
                ...
                if (isCurrent && textAnim !== 'none') {
                    if (textAnim === 'bounce') animOffsetY = Math.sin(time * 6) * 15 * scale;
                ...
                if (isAutoIntro) {
                    ...
                    let titleY = yPos;
                    if (artistStr) titleY = yPos - (artistLH / 2);
